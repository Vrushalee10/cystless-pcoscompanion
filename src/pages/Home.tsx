...
const Home = () => {
  ...
  // Use cycle intelligence for badge when available, fallback to old logic
  const getBadgeProps = () => {
    if (ci.badgeText !== "Set up cycle tracking →") {
      return { text: ci.badgeText, bg: ci.badgeBg, color: ci.badgeColor, route: "/cycle" };
    }
    if (cs === "irregular_short") return { text: "Cycle irregular · Tracking in progress", bg: "#FFF8ED", color: "#92400E", route: "/cycle" };
    if (cs === "irregular_medium") return { text: "Irregular cycle · Building your baseline", bg: "#FFF8ED", color: "#92400E", route: "/cycle" };
    if (cs === "irregular_long" || cs === "never_regular") return { text: "Cycle monitoring · See Cysta", bg: "#FCECEA", color: "#D4614F", route: "/chat" };
    ...
    return { text: "Set up cycle tracking →", bg: "#E2DDD7", color: "var(--text-muted)", route: "/cycle-setup" };
  };

  const badge = getBadgeProps();

  // Determine which insight to show - pattern insight takes priority after 3 cycles
  const showPatternInsight = ci.patternInsight !== null;
  const displayInsight = showPatternInsight
    ? { headline: "Your cycles are telling a story.", body: ci.patternInsight! }
    : insight;
  const insightLabel = showPatternInsight ? "YOUR CYCLE PATTERN" : "TODAY'S INSIGHT";
    ...
    <div className="min-h-[100dvh] bg-background flex justify-center">
      <div className="w-full max-w-[390px] min-h-[100dvh] flex flex-col px-5 pb-[80px]">
        ...
        >
          ...
          <div className="flex justify-between items-start mt-5">
            <div>
              ...
              <button
                ...
                style={{ fontSize: 13, fontWeight: 600, color: badge.color, backgroundColor: badge.bg, padding: "5px 10px", borderRadius: 100 }}
              >
                {badge.text}
              </button>
              <button
          ...
          {/* Delay callout */}
          {ci.delayMessage && (ci.delayStatus === "significantly_late" || ci.delayStatus === "very_late") && (
            <button
              onClick={() => navigate(ci.delayStatus === "very_late" ? "/chat" : "/log")}
              className="w-full mt-4 p-4 text-left"
              style={{
                borderRadius: 16,
                backgroundColor: ci.delayStatus === "very_late" ? "#FCECEA" : "#FFF8ED",
                border: `1px solid ${ci.delayStatus === "very_late" ? "#D4614F" : "#F5DEB3"}`,
              }}
            >
              <p className="font-body" style={{ fontSize: 14, fontWeight: 600, color: ci.delayStatus === "very_late" ? "#D4614F" : "#92400E" }}>
                {ci.delayStatus === "very_late" ? "⚠️ Period significantly late" : "Period late"}
              </p>
              <p className="font-body mt-1" style={{ fontSize: 13, color: "var(--text-body)", lineHeight: 1.5 }}>
                {ci.delayMessage}
              </p>
            </button>
          )}

          {/* Today's Insight / Pattern */}
          <div className="bg-card mt-6 p-6" style={{ borderRadius: 22, boxShadow: "var(--shadow-hero)" }}>
            <div className="flex items-center justify-between">
              <span className="text-label" style={{ color: showPatternInsight ? "var(--text-muted)" : "hsl(var(--primary))" }}>
                {insightLabel}
              </span>
              <span
                className="block rounded-full"
                style={{ width: 9, height: 9, backgroundColor: "hsl(var(--accent))", animation: "insightPulse 2s ease-in-out infinite" }}
              />
            </div>
            <h2 className="font-display mt-4" style={{ fontSize: 26, fontWeight: 700, color: "var(--text-ink)", lineHeight: 1.25 }}>
              {displayInsight.headline}
            </h2>
            <div className="mt-[14px] mb-[14px]" style={{ height: 3, borderRadius: 2, backgroundColor: "hsl(var(--accent) / 0.1)" }} />
            <p className="font-body" style={{ fontSize: 15, color: "var(--text-body)", lineHeight: 1.65 }}>
              {displayInsight.body}
            </p>
            <button
          ...
          {/* Today's Focus */}
          <p className="text-label mt-7" style={{ color: "var(--text-muted)" }}>TODAY'S FOCUS</p>
          <div className="bg-card mt-3 p-5" style={{ borderRadius: 18, boxShadow: "var(--shadow-card)", borderLeft: "5px solid hsl(var(--primary))" }}>
            <span className="text-label" style={{ color: "hsl(var(--primary))" }}>NUTRITION</span>
            <p className="font-body mt-2" style={{ fontSize: 14, color: "var(--text-body)", lineHeight: 1.6 }}>
              Prioritise protein at breakfast to stabilise blood sugar before 10am. Eggs, Greek yogurt, or a protein smoothie all work for your insulin resistance pattern.
            </p>
          </div>
          ...
          <button
            ...
            style={{ borderRadius: 16, padding: 18, boxShadow: "var(--shadow-card)" }}
          >
            <span style={{ fontSize: 24, marginRight: 12 }}>⌚</span>
            <div className="flex-1 min-w-0">
              <p className="font-body" style={{ fontSize: 14, fontWeight: 700, color: "var(--text-ink)" }}>Auto-track with your watch</p>
            ...
            <span className="font-body shrink-0 ml-3" style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--primary))" }}>Connect →</span>
          </button>
        </motion.div>
      </div>
